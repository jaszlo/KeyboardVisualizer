...
## Running this application on windows
...
```sh
cl .\winKeyHook.cpp  /link User32.lib
```

After that move the resulting `winKeyHook.exe` next to the `main.py`. To start the application just run `main.py`. 
```sh
python main.py
```
...
- `LAYOUT` represents the actual values provided by the keyHook executable.
